            ...
            while query < iterations:  # MaxQueries:
                if follow_baseline is not None and query < initial_points:
                    ...
                    if query >= initial_points:
                        # Extensive search
                        if query >= begin_strat and (strategy == "ExtensiveSearch"):
                            if query == begin_strat:
                                possible_queries = list(range(96))  # Example: values from 0 to 95
                                # Get the remaining queries
                                remaining_queries = list(set(possible_queries) - set(queried_points))

                                # Shuffle the remaining queries
                                random.shuffle(remaining_queries)
                                # print("queried_points ", queried_points, "\n")
                                # print("Remaining queries: ", remaining_queries, "\n")
                                
                            next_query = remaining_queries.pop()
                        
                        else:
                            next_query = UCB(mu, sigma, kappa=kappa)

                        queried_points.append(next_query)
                ...
                # MP-BO
                if query >= begin_strat and (strategy != "Vanilla BO") and (strategy != "ExtensiveSearch"):
                ...
                # Sanity check
                if strategy != "Vanilla BO" and strategy != "ExtensiveSearch":
